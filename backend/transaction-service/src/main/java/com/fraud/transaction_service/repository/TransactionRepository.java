...
public interface TransactionRepository extends JpaRepository<Transaction,Long> {
    ...
    @Query(value = """
            SELECT
              ...
              /* peak hourly count per customer within the range */
              (
                SELECT MAX(x.cnt)
                FROM (
                  SELECT COUNT(*) AS cnt
              ...
              COUNT(DISTINCT t.location)      AS uniqueCountryCount,   -- using location as “country/location”
              0                               AS uniqueIpCount,        -- you don't have ip column
              COUNT(DISTINCT t.merchant_code) AS uniqueMerchantCount,
              0                               AS riskyMerchantCount    -- you don't have merchant_risky column
